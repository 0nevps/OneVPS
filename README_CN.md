...
- WS 路径按 **`Upgrade` 头**判定——只有真正的 WebSocket 握手才会进入 Xray。其余所有请求（任意路径，包括浏览器直接 GET 该 WS 路径）一律返回 **HTTP 403 + 403 页面**（`/var/lib/onevps/sites/<域名>/index.html`），整个子域看起来像一个受限站点，而非死代理。可随时把该 `index.html` 换成自己的内容，脚本不会覆盖。
